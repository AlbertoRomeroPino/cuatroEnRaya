...
public class yaFuncionaColumna {
    ...
    public static boolean comprobadorFila(int[] ultimaFicha, String jugador, String[][] tablero) {
        ...
        for (int i = 0; i < 7 && !ganar; i++) {
            if (tablero[ultimaFicha[0]][i].equals(jugador)) {
                contador++;
            } else {
                contador = resetearContador();
            }
        ...
        contador = resetearContador();
        for (int i = 0; i < tablero.length && !ganar; i++) {
            for (int j = 0; j < tablero[i].length && !ganar; j++) {

                // Verificar diagonal hacia la derecha
                if (dentroDeLosLimites(tablero, i + 3, j + 3)) {
                    for (int l = 0; l <= 3; l++) {
                        if (tablero[i + l][j + l].equals(jugador)) {
                            contador++;
                        }else {
                            contador = resetearContador();
                        }
    ...
    public static boolean dentroDeLosLimites(String[][] tablero, int fila, int columna) {
    ...
    public static int resetearContador() {
        int contador = 0;
        return contador;
    }
